...
SLOT="0"
IUSE="caps debug doc examples force-xdg gdbm maildir pcre static unicode"
...
src_install() {
	...
	if use force-xdg ; then
		dodir /etc/zsh
		if ! grep -q '# force_xdg >>>' "${ED}"/etc/zsh/zshenv ; then
			printf '# force_xdg >>>\n' >> "${ED}"/etc/zsh/zshenv
			printf 'export ZDOTDIR="${XDG_CONFIG_HOME:-$HOME/.config}/zsh"\n' >> "${ED}"/etc/zsh/zshenv
			printf 'export HISTFILE="${XDG_STATE_HOME:-$HOME/.local/state}/zsh/history"\n' >> "${ED}"/etc/zsh/zshenv
			printf '# force_xdg <<<\n' >> "${ED}"/etc/zsh/zshenv
		fi
	else
		sed -i -e '/# force_xdg >>>/,/# force_xdg <<</d' /etc/zsh/zshenv
	fi
